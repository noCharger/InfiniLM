...
infinilm::InfinilmModel::Output VideoNSAForConditionalGeneration::forward(const infinilm::InfinilmModel::Input &input) const {
    if (input.pixel_values.has_value() && input.pixel_values.value().size() > 0) {
        ...
        for (size_t media_idx = 0; media_idx < image_req_ids.size(); ++media_idx) {
            ...
            auto grid_cpu = input.tgt_sizes.value().at(media_idx)->to(infinicore::Device::cpu());
            auto grid_count = grid_cpu->size(0);
            auto grids = reinterpret_cast<const int64_t *>(grid_cpu->data());
            bool has_video_frames = false;
            for (size_t i = 0; i < grid_count; ++i) {
                has_video_frames = has_video_frames || grids[i * 3] > 1;
            }
            ...
            for (size_t i = 0; i < bound_count; ++i) {
                ...
                if (end > start) {
                    ...
                    if (has_video_frames) {
                        visual_token_ranges.push_back(size_t(offsets[req_id]) + static_cast<size_t>(start));
                        visual_token_ranges.push_back(size_t(offsets[req_id]) + static_cast<size_t>(end));
                    }
                }
            }
        ...
        global_state::get_forward_context().mm_metadata.visual_token_ranges = std::move(visual_token_ranges);
        auto hidden_states = model_->forward_embeds(inputs_embeds, input.position_ids.value());
        global_state::get_forward_context().mm_metadata.visual_token_ranges = std::nullopt;
